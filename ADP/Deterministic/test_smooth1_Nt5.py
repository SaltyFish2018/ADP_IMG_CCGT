...
for n in range(1,100):
    ...
    # for interval in range(2, 4):
        ...
        # new5.setAttr('rhs', (-1 + df_Q_X2.loc[(interval - 1) * 18].Q_X2))
        # new6 = change.getConstrByName('new6')
        # new6.setAttr('rhs', (-1 + df_Q_X3.loc[(interval - 1) * 18].Q_X3))
        # new7 = change.getConstrByName('new7')
        # new7.setAttr('rhs', (-1 - (- 0.257 * df_Q_X4.loc[(interval - 1) * 18].Q_X4 + 0.3266 * df_Q_X5.loc[(interval - 1) * 18].Q_X5 \
        #                            + 0.6292 * df_Q_X6.loc[(interval - 1) * 18].Q_X6 - 1.63 * df_Q_X7.loc[(interval - 1) * 18].Q_X7 \
        #                            - df_decision_vars.loc[interval-1].g_CCGT)))
    ...
    from smooth1 import smooth_update
    ...
    for interval in range(1,periods):
        ...
        df_slopes.loc[interval] = np.array(smooth_update(located_bars))
