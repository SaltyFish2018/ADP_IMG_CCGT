...
types = ("P_FC", "Q_GB", "g_CCGT", "P_wcur", "P_cur", "Q_cur", "Q_waste", "SOC")#, "SOC")+ df_types.loc["SOC"].param1 * SOC\

adjustable_parameters_data = {"param1": [100, 30, 200, 200, 350, 350, 200, 0]}#130, 300, 1460, 200, 150, 350, 0, 50
...
strategy.addConstr((Q_GB + Q_HP + Q_cur) + Q_CCGT[18]+(-Q_waste) - demand_Heat[interval] == 0)
...
strategy.addConstr(Q_waste - demand_Heat[interval] <= 0)
...
for interval in range(2, periods+1):
    ...
    strategy.addConstr((P_FC + (g_CCGT * 16 - 10) + P_GRID + P_d + P_cur + (-P_wcur) \
                        - P_c - Q_HP / 4.5) + P_WT_a[interval] - demand_Elec[interval] == 0)
    strategy.addConstr((Q_GB + Q_HP + Q_cur) + Q_CCGT[18] +(-Q_waste) - demand_Heat[interval] == 0)
