...
for m, ii in df_median_decision.groupby(level='intervals_median'):
    strategy.addConstr(gp.quicksum(ii.P_FC + (ii.g_CCGT * 16 - 10) + ii.P_GRID + ii.P_d + ii.P_cur + (-ii.P_wcur) \
                                   - ii.P_c - ii.Q_HP / 4.5) + P_WT_a[i+m-1] - demand_Elec[i+m-1] == 0)
    strategy.addConstr(gp.quicksum(ii.Q_GB + ii.Q_HP + ii.Q_cur - ii.Q_waste) + df_median_CCGT.loc[18 * m].Q_CCGT - demand_Heat[i+m-1] == 0)
...
for m, ii in df_median_decision.groupby(level='intervals_median'):
    ...
    if m ==1:
        strategy.addConstr(gp.quicksum(ii.SOC - 7.5 - 0.25 * (ii.P_c * 0.9 - ii.P_d / 0.9)) == 0)
    else:
        strategy.addConstr(gp.quicksum(ii.SOC - 0.25 * (ii.P_c * 0.9 - ii.P_d / 0.9))-df_median_decision.loc[m-1].SOC == 0)
...
for m in moving_size:
    ...
    strategy.addConstr(df_median_decision.loc[m].Q_waste - demand_Heat[i + m - 1] <= 0)
...
for i in range(2, periods-p+2):
    ...
    # Create decision variables DataFrame
    df_median_decision = DataFrame({'P_FC': P_FC, 'g_CCGT': g_CCGT, 'P_GRID': P_GRID, 'P_c': P_c, 'u_c': u_c, 'P_d': P_d, 'u_d': u_d,
         'P_wcur': P_wcur, 'P_cur': P_cur, 'Q_GB': Q_GB, 'Q_HP': Q_HP, 'Q_cur': Q_cur, 'SOC': SOC, 'Q_waste': Q_waste})
    ...
    for m, ii in df_median_decision.groupby(level='intervals_median'):
        ...
        if m == 1:
            strategy.addConstr(gp.quicksum(ii.SOC - 0.25 * (ii.P_c * 0.9 - ii.P_d / 0.9))- df_decision_vars.loc[i - 1].SOC == 0)
        else:
            strategy.addConstr(gp.quicksum(ii.SOC - 0.25 * (ii.P_c * 0.9 - ii.P_d / 0.9)) - df_median_decision.loc[m - 1].SOC == 0)
