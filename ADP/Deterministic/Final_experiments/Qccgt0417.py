...
for n in range(1,50):
    ...
    for Interval in Intervals_CCGT:
        # if Interval ==1:
        #     strategy.addConstr(0.4031*Q_X1[Interval]+ 0.3656 * Q_X2[Interval]+ 0.06311 * Q_X3[Interval] + 0.2087 * Q_X4[Interval] >= 21)#.538805809250476
        #     # strategy.addConstr(Q_X2[Interval] == 18)#.878339830314335
        #     # strategy.addConstr(Q_X3[Interval] == 24)#.415138314504095
        #     # strategy.addConstr(Q_X4[Interval] == 26)#.141357955320913
        #     # strategy.addConstr(Q_X5[Interval] == 28.24599901309491)
        #     # strategy.addConstr(Q_X6[Interval] == 21.07870976146272)
        #     # strategy.addConstr(Q_X7[Interval] == 23.41235478654693)
        # else:
        if Interval >=2:
    ...
    QCCGT_update_a = (decisions[interval-1,15] - 15) // ub
    if QCCGT_update_a == 5:
        a_loc[interval-1] = 4
    elif QCCGT_update_a == -1:
        a_loc[interval-1] = 0
    else:
        a_loc[interval-1] = QCCGT_update_a
    ...
    for interval in range(2, periods+1):
        ...
        for Interval in Intervals_CCGT:
            strategy.addConstr(0.4031 * Q_X1[Interval] + 0.3656 * Q_X2[Interval] + 0.06311 * Q_X3[Interval] + 0.2087 * Q_X4[Interval] <= 50,'Q_ccgt_up')
            strategy.addConstr(0.4031 * Q_X1[Interval] + 0.3656 * Q_X2[Interval] + 0.06311 * Q_X3[Interval] + 0.2087 * Q_X4[Interval] >= 15)
        ...
        strategy.update()
        # Ramping Constraints
        strategy.addConstr(g_CCGT - decisions[interval-2,1] <= 1.5,'g_ccgt_up')
        strategy.addConstr(g_CCGT - decisions[interval-2,1] >= -1.5,'g_ccgt_down')
        ...
        # g_ccgt_up = change.getConstrByName('g_ccgt_up')
        # g_ccgt_up.setAttr('rhs',(2.5+decisions[interval-2,1]))
        # g_ccgt_down = change.getConstrByName('g_ccgt_down')
        # g_ccgt_down.setAttr('rhs',(-2.5+decisions[interval-2,1]))

        # Q_ccgt_up = change.getConstrByName('Q_ccgt_up')
        # Q_ccgt_up.setAttr('rhs',51)
        ...
        Partials[interval-1]= (S1 - S2)/ (0.4031 + 0.3656 + 0.06311 + 0.2087)
        # Partials[interval - 1] = 1 * (S1 - S2) / (Q_aug_CCGT[interval * 18 - 1, 7] - Q_temp[interval * 18 - 1, 7]+0.01)
    ...
    b = 5       #tunable parameters
    step_size = b/(b+n-1)       # Generalized Harmonic Stepsizes

    for interval in range(1,periods):
        a = a_loc[interval-1]
        # updated_slope = step_size * Partials[interval]+ (1-step_size) * slopes[interval-1, int(a)]
        updated_slope = step_size * Partials[interval:].max() + (1 - step_size) * slopes[interval - 1, int(a)]
...
#
# outputpath1 = 'D:\\Nuts\CCGT\ADP\Deterministic\确定性结果图\\0412Q_CCGT_Cold.csv'
# Q_CCGT_all = DataFrame(Q_aug_CCGT[:,7], columns=['Q_ccgt'])
