...
for i in range(1,31):
    ...
    strategy = gp.Model("Stochastic milp")
    ...
    df_decision_vars = DataFrame(
        {'P_FC': P_FC, 'g_CCGT': g_CCGT, 'P_GRID': P_GRID, 'P_c': P_c, 'u_c': u_c, 'P_d': P_d, 'u_d': u_d, 'P_wcur': P_wcur,
         'P_cur': P_cur, 'Q_GB': Q_GB, 'Q_HP': Q_HP, 'Q_cur': Q_cur, 'SOC': SOC, 'Q_waste':Q_waste}, index=range(1,periods+1))

    df_Q_X = DataFrame({'Q_X1':Q_X1, 'Q_X2':Q_X2, 'Q_X3':Q_X3, 'Q_X4':Q_X4, 'Q_X5':Q_X5, 'Q_X6':Q_X6, 'Q_X7':Q_X7}, index=range(1,periods+1))
    ...
    for interval, i in df_decision_vars.groupby(level='intervals'):
        strategy.addConstr(gp.quicksum(i.P_FC + (i.g_CCGT * 16 - 10) + i.P_GRID + i.P_d * i.u_d + i.P_cur + (
            -i.P_wcur) - i.P_c * i.u_c - i.Q_HP / 4.5) + P_WT_a[interval-1] - demand_Elec[interval-1] == 0)
    for interval in intervals:
        strategy.addConstr( (Q_GB[interval] + Q_HP[interval] +  Q_cur[interval]) + 0.4031 * Q_X1[18*interval] \
                            ...
                            + (-Q_waste[interval])- demand_Heat[interval-1] == 0)
    ...
    for interval in intervals:
        strategy.addConstr(df_decision_vars.loc[interval].P_wcur - P_WT_a[interval-1] <= 0)
        strategy.addConstr(df_decision_vars.loc[interval].P_cur - demand_Elec[interval-1] <= 0)
        strategy.addConstr(df_decision_vars.loc[interval].Q_cur - demand_Heat[interval-1] <= 0)
        strategy.addConstr(df_decision_vars.loc[interval].Q_waste - demand_Heat[interval-1] <= 0)
    ...
    for interval in intervals:
        step_cost = 0.25 * (df_types.loc["P_FC"].param1 * df_decision_vars.loc[interval,'P_FC']+
                            ...
                            + 1000 * price[interval-1] * df_decision_vars.loc[interval,'P_GRID'])\
    ...
    for interval in intervals:
        df_GRID_PRICE_sol.loc[interval].GRID_PRICE = 0.25*1000 * df_decision_vars_sol.loc[interval].P_GRID * price[interval-1]
    ...
    Q_CCGT_sol = {}
    for Interval in Intervals_CCGT:
        Q_CCGT_All_sol[Interval] = 0.4031 * Q_X1_sol[Interval] + 0.3656 * Q_X2_sol[Interval] + 0.06311 * Q_X3_sol[
            Interval] + 0.2087 * Q_X4_sol[Interval]
        if Interval % 18 == 0:
            Q_CCGT_sol[Interval / 18] = 0.4031 * Q_X1_sol[Interval] + 0.3656 * Q_X2_sol[Interval] + 0.06311 * Q_X3_sol[
                Interval] + 0.2087 * Q_X4_sol[Interval]
...
# Save results
np.save('D:\\Nuts\CCGT\ADP\Stochastic\\milp_robust_cost_e3.npy', Cost)
...
# outputpath = 'D:\\Nuts\CCGT\MILP\\0405MILP_result.csv'
# df_decision_vars_sol.to_csv(outputpath, sep=',', index=True, header=True)
#
# outputpath1 = 'D:\\Nuts\CCGT\MILP\\0405MILP_Q_CCGT_all.csv'
