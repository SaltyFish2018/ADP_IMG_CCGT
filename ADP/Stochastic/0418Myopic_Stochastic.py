...
scenarios = np.load("D:\\Nuts\CCGT\ADP\Stochastic\\test_scenarios_1.npy")
# # Step 3: Model Setup
...
for i in range(1,201):
        ...
        if Interval >= 5 and Interval <= 18:
            if Interval == 5:
                strategy.addConstr(
                    ...
                    Q_CCGT[Interval - 3] - 0.257 * Q_CCGT[Interval - 4] - (0.2087 * g_CCGT) == 0)
            elif Interval == 6:
                strategy.addConstr(
                    Q_CCGT[Interval] - 1.63 * Q_CCGT[Interval - 1] + 0.6292 * Q_CCGT[Interval - 2] + 0.3266 *
                    Q_CCGT[Interval - 3] - 0.257 * Q_CCGT[Interval - 4] - ((0.2087 + 0.0631) * g_CCGT) == 0)
            elif Interval == 7:
                strategy.addConstr(
                    Q_CCGT[Interval] - 1.63 * Q_CCGT[Interval - 1] + 0.6292 * Q_CCGT[Interval - 2] + 0.3266 *
                    Q_CCGT[Interval - 3] - 0.257 * Q_CCGT[Interval - 4]- ((0.2087 + 0.0631 + 0.3656) * g_CCGT) == 0)
            else:
                strategy.addConstr(
                    Q_CCGT[Interval] - 1.63 * Q_CCGT[Interval - 1] + 0.6292 * Q_CCGT[Interval - 2] + 0.3266 *
                    Q_CCGT[Interval - 3] - 0.257 * Q_CCGT[Interval - 4] - ((0.2087 + 0.0631 + 0.3656 + 0.4031) * g_CCGT) == 0)
    ...
    strategy.addConstr((Q_GB + Q_HP + Q_cur) + Q_CCGT[18]+(-Q_waste) - demand_Heat[interval-1] == 0)
    ...
    step_cost = 0.25 * ((df_types.loc["P_FC"].param1 * P_FC \
                        ...
                        + df_types.loc["Q_waste"].param1 * Q_waste\
                        + df_types.loc["SOC"].param1 * SOC\
                        + 1000 * price[interval-1] * P_GRID)
    ...
    df_GRID_PRICE.loc[interval].GRID_PRICE = 0.25 * 1000 * price[interval-1] * P_GRID.x
    # print(df_GRID_PRICE)
    ...
    for interval in range(2, periods+1):
        ...
        for Interval in Intervals_CCGT:
            if Interval == 1:
                strategy.addConstr(Q_CCGT[Interval] - 1.63 * df_Q_CCGT.loc[interval * 18 - 18].Q_CCGT + 0.6292 * df_Q_CCGT.loc[interval * 18 - 19].Q_CCGT \
                                   + 0.3266 * df_Q_CCGT.loc[interval * 18 - 20].Q_CCGT - 0.257 * df_Q_CCGT.loc[interval * 18 - 21].Q_CCGT \
                ...
                strategy.addConstr(
                    Q_CCGT[Interval] - 1.63 * Q_CCGT[Interval - 1] + 0.6292 * Q_CCGT[Interval - 2] + 0.3266 * Q_CCGT[
                        Interval - 3] - 0.257 * df_Q_CCGT.loc[interval * 18 - 18].Q_CCGT \
                    - (0.2087 + 0.0631 + 0.3656 + 0.4031) * df_decision_vars.loc[interval - 1].g_CCGT == 0)
            elif Interval == 5:
                strategy.addConstr(
                    Q_CCGT[Interval] - 1.63 * Q_CCGT[Interval - 1] + 0.6292 * Q_CCGT[Interval - 2] + 0.3266 * Q_CCGT[
        ...
        strategy.update()
        # 2) Equations
        strategy.addConstr((P_FC + (g_CCGT * 16 - 10) + P_GRID + P_d + P_cur + (-P_wcur) \
                            - P_c - Q_HP / 4.5) + P_WT_a[interval-1] - demand_Elec[interval-1] == 0)
        strategy.addConstr((Q_GB + Q_HP + Q_cur) + Q_CCGT[18] +(-Q_waste) - demand_Heat[interval-1] == 0)
        ...
        strategy.update()
        # Battery Constraints
        strategy.addConstr(SOC - df_decision_vars.loc[interval - 1].SOC - 0.25 * (P_c * 0.9 - P_d / 0.9) == 0)
        strategy.addConstr(u_c + u_d <= 1)
        strategy.addConstr(P_c - u_c * 3 <= 0)
        strategy.addConstr(P_d - u_d * 3 <= 0)
        strategy.addConstr(SOC >= 1.5)
...
# print(np.mean(Cost))
np.save('D:\\Nuts\CCGT\ADP\Stochastic\\myopic_test_cost.npy', Cost)
...
# outputpath = 'D:\\Nuts\CCGT\ADP\Deterministic\Q_ccgt\\04Myopic_result.csv'
# df_decision_vars.to_csv(outputpath, sep=',', index=True, header=True)
# #
# outputpath1 = 'D:\\Nuts\CCGT\ADP\Deterministic\Q_ccgt\\04Myopic_Q_CCGT_all.csv'
...
# 显示标题
ax1.set_title('Myopic Policy Power Assignment', fontname='Times New Roman',fontsize = 20,y = 1.05)
...
ax.legend()
# 显示标题
ax.set_title('Myopic Policy Heat Assignment',fontname='Times New Roman', fontsize = 20, y = 1.05)
# 显示网格
fig.tight_layout()
plt.grid(axis="y", linestyle='-.')
