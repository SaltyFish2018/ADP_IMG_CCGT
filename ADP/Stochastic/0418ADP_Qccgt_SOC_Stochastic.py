...
scenarios = np.load("scenarios.npy")
...
for epoch in range(1,1001):
    ...
    demand_Elec = scenarios[epoch-1,0,:]
    demand_Heat = scenarios[epoch-1,1,:]
    # mean=0, variance=0.00001
    price = scenarios[epoch-1,2,:]
    # shape=1, scale=0.1
    P_WT_a = scenarios[epoch-1,3,:]
    ...
    step_size_CCGT = b_CCGT/(b_CCGT+epoch-1)       # Generalized Harmonic Stepsizes

    b_SOC = 5
    step_size_SOC = b_SOC/(b_SOC+epoch-1)
...
np.save('D:\\Nuts\CCGT\ADP\Stochastic\\adp_cost.npy', Cost)
np.save('D:\\Nuts\CCGT\ADP\Stochastic\\slopes_CCGT.npy', slopes_CCGT)
np.save('D:\\Nuts\CCGT\ADP\Stochastic\\slopes_SOC.npy', slopes_SOC)
# print(np.mean(Cost[epoch-50:epoch]))
# print(slopes_SOC)
# print(slopes_CCGT)

# outputpath = 'D:\\Nuts\CCGT\ADP\Stochastic\\Cost_Stochastic.csv'
# df_Cost = DataFrame(columns=['Cost'],data=Cost,index=range(1,epoch+1))
# df_Cost.to_csv(outputpath, sep=',', header=True)
