...
for epoch in range(1,50):
    ...
    # mean=0, variance=0.1
    demand_Elec = Guass_Generation(demand_Elec_original, 0, 9)
    demand_Heat = Guass_Generation(demand_Heat_original, 0, 9)
    ...
    for i in range(2, periods-p+2):
        ...
        df_median_decision = DataFrame(
            {'P_FC': P_FC, 'g_CCGT': g_CCGT, 'P_GRID': P_GRID, 'P_c': P_c, 'u_c': u_c, 'P_d': P_d, 'u_d': u_d,
             'P_wcur': P_wcur,
             'P_cur': P_cur, 'Q_GB': Q_GB, 'Q_HP': Q_HP, 'Q_cur': Q_cur, 'SOC': SOC, 'Q_waste': Q_waste}, index= moving_size)
